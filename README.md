...
Програма предназначена для автоматической регистрации талона, без участия пользователя, т.е. пользователь освобождается постоянного мониторинга сайта.
...
7. Программ запросит время/час до которого нужно взять талон. Рекомендуем оставить его пустым и просто нажать Enter, тем самым программа возмет любой доступный талон. Данная функция экспериментальная и все еще не отлажена должным образом.
8. Укажите номер СЦ МВС и нажмите клавишу Enter (чтобы узнать номер СЦ, на карте можете открыть нужный для вас СЦ, в заголовке окна будет указан номер);
9. После указания всех данных из пп.5, программа автоматически начинает сканирование.
